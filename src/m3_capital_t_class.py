...
class CapitalT(object):
    ...
    *** See the PDFs, especially dimensions.pdf, to help you understand this.
    ...
    def clone(self):
        ...
        #######################################################################
        new = self.h_rect.get_center()
        new1 = self.h_rect.get_width()
        new2 = self.v_rect.get_height()
        new3 = self.v_rect.get_width()
        final = CapitalT(new, new1, new2, new3)
        final.set_colors(self.h_rect.fill_color, self.h_rect.outline_color)
        return final
